...
namespace D_RekSai
{
    internal static class Program
    {
        ...
        private static void GameOnOnGameLoad()
        {
            ...
            _config = new Menu("VIP挖掘机Q群438230879", "VIP挖掘机Q群438230879", true);

            _config.AddSubMenu(new Menu("    走砍设置    ", "Orbwalking"));
            ...
            _config.AddSubMenu(new Menu("    清线设置    ", "Lane"));
            ...
            _config.SubMenu("Lane").AddItem(new MenuItem("UseItemslane", "Use Items")).SetValue(true);

            _config.AddSubMenu(new Menu("    清野设置    ", "Jungle"));
            ...
            _config.SubMenu("Jungle").AddItem(new MenuItem("UseEJungle", "Use E")).SetValue(true);
            _config.SubMenu("Jungle").AddItem(new MenuItem("UseItemsjungle", "Usem Items")).SetValue(true);

            _config.AddSubMenu(new Menu("    自动使用    ", "AUTOTO"));
            _config.SubMenu("AUTOTO").AddItem(new MenuItem("AutoW", "Auto W")).SetValue(true);
            _config.SubMenu("AUTOTO").AddItem(new MenuItem("AutoWHP", "Use W if HP is <= ")).SetValue(new Slider(25, 1, 100));
            _config.SubMenu("AUTOTO").AddItem(new MenuItem("AutoWMP", "Use W if Fury is >= ")).SetValue(new Slider(100, 1, 100));
            _config.SubMenu("AUTOTO").AddItem(new MenuItem("Inter_W", "Use W to Interrupter")).SetValue(true);
            _config.SubMenu("AUTOTO").AddItem(new MenuItem("turnburrowed", "自动潜地|非启动走砍按键时候")).SetValue(true);

            _config.AddSubMenu(new Menu("    击杀设置    ", "Ks"));
            ...
            _config.AddSubMenu(new Menu("    逃跑设置    ", "Misc"));
            _config.SubMenu("Misc").AddItem(new MenuItem("escapeterino", "逃跑按键")).SetValue(new KeyBind("Z".ToCharArray()[0], KeyBindType.Press));

            _config.AddSubMenu(new Menu("    物品使用    ", "items"));
            ...
            _config.SubMenu("items").AddSubMenu(new Menu("    防守物品    ", "Deffensive"));
            ...
            _config.AddSubMenu(new Menu("    惩戒设置    ", "Smite"));
            ...
            var WardMenu = _config.AddSubMenu(new Menu("    自动眼位    ", "AutoWard"));
            {
            ...
            var SkinMenu = _config.AddSubMenu(new Menu("    换肤设置    ", "换肤设置"));
            {
                SkinMenu.AddItem(new MenuItem("EnableSkin", "启动换肤").SetValue(false));
                SkinMenu.AddItem(new MenuItem("SkinSelect", "选择皮肤").SetValue(new StringList(new[] { "经典", "上古战魂", "泳池派对" })));
            }
        ...
        private static void Game_OnGameUpdate(EventArgs args)
        {
            ...
            if ((_orbwalker.ActiveMode == Orbwalking.OrbwalkingMode.Mixed || _config.Item("harasstoggle").GetValue<KeyBind>().Active) && _orbwalker.ActiveMode != Orbwalking.OrbwalkingMode.Combo)
            {
                Harass();
            }
            ...
            if (_config.Item("AutoW").GetValue<bool>() && (_config.Item("turnburrowed").GetValue<bool>() && _orbwalker.ActiveMode != Orbwalking.OrbwalkingMode.Combo || _orbwalker.ActiveMode != Orbwalking.OrbwalkingMode.Mixed || !_config.Item("harasstoggle").GetValue<KeyBind>().Active || _orbwalker.ActiveMode != Orbwalking.OrbwalkingMode.LaneClear || !_config.Item("escapeterino").GetValue<KeyBind>().Active))
            {
                AutoW();
            }
            ...
            if ((_orbwalker.ActiveMode != Orbwalking.OrbwalkingMode.Combo || _orbwalker.ActiveMode != Orbwalking.OrbwalkingMode.Mixed || !_config.Item("harasstoggle").GetValue<KeyBind>().Active || _orbwalker.ActiveMode != Orbwalking.OrbwalkingMode.LaneClear || !_config.Item("escapeterino").GetValue<KeyBind>().Active) && _config.Item("turnburrowed").GetValue<bool>() && !IsBurrowed())
            {
                autoburrowed();
            }
        }
        ...
        {
            AutoWard.Enable = _config.GetBool("AutoWard");
            AutoWard.AutoBuy = _config.GetBool("AutoBuy");
            AutoWard.AutoPink = _config.GetBool("AutoPink");
            AutoWard.OnlyCombo = _config.GetBool("AutoWardCombo");
            AutoWard.InComboMode = _orbwalker.ActiveMode == Orbwalking.OrbwalkingMode.Combo;
        }

        private static void Skin()
        {
            if (_config.Item("EnableSkin").GetValue<bool>())
            {
                ObjectManager.Player.SetSkin(ObjectManager.Player.CharData.BaseSkinName, _config.Item("SkinSelect").GetValue<StringList>().SelectedIndex);
            }
            else if (!_config.Item("EnableSkin").GetValue<bool>())
            {
            ...
            if (IsBurrowed())
            {
                if (_config.Item("UseEBCombo").GetValue<bool>())
                {
                    ...
                    if (_be.IsReady() && te.IsValidTarget(_be.Range + _bw.Range) && _player.Distance(te) > _q.Range)
                    {
                        ...
                        if (predE.Hitchance >= HitChance.High) _be.Cast(predE.CastPosition.Extend(_player.ServerPosition, -100));
                    }
                }
