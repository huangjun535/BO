...
namespace Marksman.Orb
{
    ...
    public static class Orbwalking
    {
        ...
        {
            "dariusnoxiantacticsonh", "fioraflurry", "garenq", "gravesmove", "hecarimrapidslash", "jaxempowertwo",
            "jaycehypercharge", "leonashieldofdaybreak", "luciane", "monkeykingdoubleattack", "mordekaisermaceofspades",
            "nasusq", "nautiluspiercinggaze", "netherblade", "gangplankqwrapper", "powerfist", "renektonpreexecute",
        ...
        public static bool CanAttack()
        {
            ...
            {
                var attackDelay = 1.0740296828d * 1000 * (int)Orbwalker.AttackSpeedDelay - 716.2381256175d;
            ...
            return LeagueSharp.Common.Utils.GameTimeTickCount + Game.Ping / 2 + 25 >= LastAATick + Orbwalker.AttackSpeedDelay * 1000;
        }
        ...
        public class Orbwalker
        {
            ...
            /// </summary>
            private Vector3 _MarksmanOrbwalkingPoint;
            ...
            public Orbwalker(Menu attachToMenu)
            {
                ...
                /* Delay sliders */
                config.AddItem(new MenuItem("ExtraWindup", "Extra windup time").SetShared().SetValue(new Slider(80, 0, 200)));
                ...
                config.AddItem(new MenuItem("Freeze", "Freeze").SetShared().SetValue(new KeyBind('N', KeyBindType.Press)));
                ...
                config.Item("StillCombo").ValueChanged += (sender, args) => { Move = !args.GetNewValue<KeyBind>().Active; };
            ...
            public void SetMarksmanOrbwalkingPoint(Vector3 point)
            {
                _MarksmanOrbwalkingPoint = point;
            }
                    ...
                    ObjectManager.Get<Obj_AI_Minion>()
                        .Any(
                            minion =>
                                ...
                                && HealthPrediction.LaneClearHealthPrediction(minion, (int)(AttackSpeedDelay * 1000 * LaneClearWaitTimeMod), FarmDelay) <= Player.GetAutoAttackDamage(minion));
            }
                        ...
                        .Any(
                            minion =>
                                ...
                                HealthPrediction.LaneClearHealthPrediction(
                                    minion,
                                    (int)
                                        (Orbwalker.AttackSpeedDelay * 1000 +
                                         (Player.IsMelee
                        ...
                        if (minions.Any())
                        {
                            ...
                            if (turretMinion != null)
                            {
                                ...
                                if (hpLeft == 0 && turretAttackCount != 0 && hpLeftBeforeDie != 0)
                                {
                                    ...
                                    var timeUntilAttackReady = LastAATick + (int)(Orbwalker.AttackSpeedDelay * 1000) >
                                                               LeagueSharp.Common.Utils.GameTimeTickCount + Game.Ping / 2 + 25
                                        ? LastAATick + (int)(Orbwalker.AttackSpeedDelay * 1000) -
                                          (LeagueSharp.Common.Utils.GameTimeTickCount + Game.Ping / 2 + 25)
                                        : 0;
                                    var timeToLandAttack = Player.IsMelee
                                        ...
                                        : Player.AttackCastDelay * 1000 +
                                          ...
                                          Player.BasicAttack.MissileSpeed;
                                    if (hits >= 1 &&
                                        hits * Orbwalker.AttackSpeedDelay * 1000 + timeUntilAttackReady + timeToLandAttack <
                                        timeBeforeDie)
                                    {
                                ...
                                {
                                    if (1f / Orbwalker.AttackSpeedDelay >= 1f &&
                                        (int)(turretAttackCount * closestTower.AttackDelay / Orbwalker.AttackSpeedDelay) *
                                        Player.GetAutoAttackDamage(lastminion) > lastminion.Health)
                                    {
                                        return lastminion;
                                    }
                                    if (minions.Count() >= 5 && 1f / Orbwalker.AttackSpeedDelay >= 1.2)
                                    {
                                        return lastminion;
                                    }
                                }
                        ...
                        result = (from minion in
                            ObjectManager.Get<Obj_AI_Minion>()
                                .Where(
                                  ...
                                  let predHealth =
                                      HealthPrediction.LaneClearHealthPrediction(
                                          minion, (int)(Orbwalker.AttackSpeedDelay * 1000 * LaneClearWaitTimeMod), FarmDelay)
                                  where
            ...
            private bool ShouldAttackMinion(Obj_AI_Minion minion, bool includeBarrel = false)
            {
                ...
                {
                    return config.Item("AttackBarrel").GetValue<bool>() &&
                           minion.CharData.BaseSkinName == "gangplankbarrel" && minion.IsHPBarRendered;
                }
                ...
                return (config.Item("AttackPetsnTraps").GetValue<bool>() || MinionManager.IsMinion(minion)) &&
            ...
            private void GameOnOnGameUpdate(EventArgs args)
            {
                ...
                try
                {
                    ...
                    Orbwalk(
                        target, _MarksmanOrbwalkingPoint.To2D().IsValid() ? _MarksmanOrbwalkingPoint : Game.CursorPos,
            ...
            private void DrawingOnOnDraw(EventArgs args)
            {
                ...
                if (config.Item("AACircle2").GetValue<Circle>().Active)
                {
                    ...
                    {
                        Render.Circle.DrawCircle(
                            target.Position, GetAttackRange(target), config.Item("AACircle2").GetValue<Circle>().Color,
                            config.Item("AALineWidth").GetValue<Slider>().Value);
                    }
                }
                ...
                if (config.Item("LastHitHelper").GetValue<bool>())
                {
                    foreach (var minion in
                        ObjectManager.Get<Obj_AI_Minion>()
                            .Where(
