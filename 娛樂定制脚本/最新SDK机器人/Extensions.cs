...
namespace Blitzcrank
{
    internal static class Extensions
    {
        ...
        {
            return target.IsValidTarget(rangeCheck ? SpellManager.R.Range : float.MaxValue) && target.GetRDamage() > target.Health + target.MagicalShield + target.HPRegenRate && !Invulnerable.Check(target, DamageType.Magical, false);
        }

        internal static CastStates CastQ(Obj_AI_Base target)
        {
            //타겟이 스펠면역일 경우 리턴
