        ...
        private static void OnEnemyGapcloser(ActiveGapcloser gapcloser, SpellSlot slot)
        {
            if (MenuExtensions.GetItemValue<bool>("dz191.vhr.misc.general.antigp") && Variables.spells[slot].IsReady())
            {
                LeagueSharp.Common.Utility.DelayAction.Add(MenuExtensions.GetItemValue<Slider>("dz191.vhr.misc.general.antigpdelay").Value,
                    () =>
                    {
                        if (gapcloser.Sender.IsValidTarget(Variables.spells[SpellSlot.E].Range) && MenuExtensions.GetItemValue<bool>("dz191.vhr.misc.general.antigp") && (Variables.spells[slot].IsReady()))
                        {
                            switch (slot)
                            {
                                case SpellSlot.Q:
                                    ...
                                    if (backOut.IsSafe())
                                    {
                                        if (gapcloser.Start.Distance(ObjectManager.Player.ServerPosition) > gapcloser.End.Distance(ObjectManager.Player.ServerPosition))
                                        {
