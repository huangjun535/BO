namespace YuLeGraves
{
    ...
    internal class Program
    {
        ...
        private static void GameOnOnGameLoad()
        {
            ...
            Config = new Menu("新冠军-男枪BUG版本", "新冠军-男枪BUG版本", true);
            Config.SetMenuColor(Color.Yellow);

            Config.AddSubMenu(new Menu("Orbwalking", "Orbwalking"));
            Orbwalker = new Orbwalking.Orbwalker(Config.SubMenu("Orbwalking"));
            Config.SubMenu("Orbwalking").AddItem(new MenuItem("FleeKEY", "翻墙按键", true).SetValue(new KeyBind('Z', KeyBindType.Press)));
        ...
        private static void OnUpdate(EventArgs args)
        {
            ...
            if (Config.SubMenu("Skin").Item("EnableSkin").GetValue<bool>())
            {
                ObjectManager.Player.SetSkin(ObjectManager.Player.ChampionName, Config.SubMenu("Skin").Item("SkinSelect").GetValue<StringList>().SelectedIndex);
            }
            else if (Config.SubMenu("Skin").Item("EnableSkin").GetValue<bool>())
            {
