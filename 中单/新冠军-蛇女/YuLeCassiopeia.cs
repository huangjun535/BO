namespace YuLeCassiopeia
{
    ...
    internal class YuLeCassiopeia
    {
        ...
        {
            Movement.SkillshotType CoreType2 = Movement.SkillshotType.SkillshotLine;
            bool aoe2 = false;
            ...
            var predInput2 = new Movement.PredictionInput
            {
                Aoe = aoe2,
            ...
            var poutput2 = Movement.Prediction.GetPrediction(predInput2);

            if (poutput2.Hitchance >= Movement.HitChance.VeryHigh)
                QWER.Cast(poutput2.CastPosition);
            else if (predInput2.Aoe && poutput2.AoeTargetsHitCount > 1 && poutput2.Hitchance >= Movement.HitChance.High)
            {
                QWER.Cast(poutput2.CastPosition);
            }
        }
        ...
        internal void OnLoader(EventArgs args)
        {
        ...
        private void LastHitLogic()
        {
            if(this.Player.ManaPercent >= Config.Instance.Menu.Item("LastHitMana").GetValue<Slider>().Value)
            {
                if (Config.Instance["UseELastHit"] && E.IsReady())
                {
                    var minionE = MinionManager.GetMinions(this.E.Range).FirstOrDefault(x => this.E.GetDamage(x) > x.Health);

                    if (minionE != null && minionE.IsValidTarget(E.Range))
                    {
                        this.E.CastOnUnit(minionE);
                    }
                }
        ...
        {
            AutoWard.Enable = Config.Instance.Menu.GetBool("AutoWard");
            AutoWard.AutoBuy = Config.Instance.Menu.GetBool("AutoBuy");
            AutoWard.AutoPink = Config.Instance.Menu.GetBool("AutoPink");
            AutoWard.OnlyCombo = Config.Instance.Menu.GetBool("AutoWardCombo");
            AutoWard.InComboMode = Orbwalker.ActiveMode == Orbwalking.OrbwalkingMode.Combo;

            if (Config.Instance.Menu.SubMenu("Skin").Item("EnableSkin").GetValue<bool>())
            {
                ObjectManager.Player.SetSkin(ObjectManager.Player.ChampionName, Config.Instance.Menu.SubMenu("Skin").Item("SkinSelect").GetValue<StringList>().SelectedIndex);
            }
            else if (Config.Instance.Menu.SubMenu("Skin").Item("EnableSkin").GetValue<bool>())
            {
