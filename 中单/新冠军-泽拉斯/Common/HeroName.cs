...
namespace CNLib {
	...
	public static class HeroName {
		...
		{
			new HeroNameEntity("Aatrox", "亚托克斯", new[] { "剑魔" }),
			...
			new HeroNameEntity("Annie", "安妮", new[] { "火女", "黑暗之女" }),
			...
			new HeroNameEntity("Cassiopeia", "蛇魔之拥", new[] { "蛇女", "卡西奥佩娅" }),
			...
			new HeroNameEntity("Jarvan IV", "德玛西亚皇子", new[] { "嘉文四世" }),
			...
			new HeroNameEntity("Tahm Kench", "塔姆", new[] { "河流之王" }),
		...
		public static string GetHeroName(string ChampionName) {
			return HeroNameList.Find(h => h.ChampionName == ChampionName)?.CNName;
		}
