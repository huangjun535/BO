namespace YuLeVelkoz
{
    ...
    class Velkoz
    {
        ...
        public void Load()
        {
            ...
            Config.SubMenu("Q 设置").AddItem(new MenuItem("QHarassMana", "最低骚扰蓝量", true).SetValue(new Slider(30, 100, 0)));
            ...
            Config.SubMenu("W 设置").AddItem(new MenuItem("WMana", "最低清线蓝量", true).SetValue(new Slider(80, 100, 0)));
            Config.SubMenu("W 设置").AddItem(new MenuItem("WLCminions", "最低命中数", true).SetValue(new Slider(2, 10, 0)));
            ...
            Config.SubMenu("E 设置").AddItem(new MenuItem("EMana", "最低清线蓝量", true).SetValue(new Slider(80, 100, 0)));
            Config.SubMenu("E 设置").AddItem(new MenuItem("ELCminions", " 最低命中数", true).SetValue(new Slider(2, 10, 0)));
            Config.SubMenu("E 设置").AddItem(new MenuItem("jungleE", "清野 E", true).SetValue(true));
            Config.SubMenu("E 设置").AddItem(new MenuItem("EInterrupter", "自动 E 打断", true).SetValue(true));
            Config.SubMenu("E 设置").SubMenu("E Gap Closer").AddItem(new MenuItem("EmodeGC", "E反突进模式", true).SetValue(new StringList(new[] { "Dash end position", "Player position", "Prediction" }, 0)));
        ...
        private void Game_OnGameUpdate(EventArgs args)
        {
            ...
            if (Player.IsChannelingImportantSpell())
            {
                if (Config.Item("autoR", true).GetValue<bool>())
                {
                    ...
                    if (t.IsValidTarget() && Common.ValidUlt(t))
                    {
        ...
        private void LogicR()
        {
            ...
            if (t.IsValidTarget()  && Player.CountEnemiesInRange(400) == 0 && !Player.UnderTurret(true))
            {
                ...
                if (distance > 900 && Common.CanMove(t))
                {
                    float adjust = (R.Range - distance) / 600;
                    rDmg = rDmg * adjust;
                }

                if(rDmg > t.Health && Common.ValidUlt(t))
                {
                    R.Cast(t);
                }
            }
        ...
        private void LogicE()
        {
            ...
            else if (Program.LaneClear && Player.ManaPercent > Config.Item("EMana", true).GetValue<Slider>().Value && Config.Item("farmE", true).GetValue<bool>() && Player.Mana > RMANA + EMANA)
            {
                ...
                if (farmPosition.MinionsHit >= Config.Item("ELCminions", true).GetValue<Slider>().Value)
        ...
        private void LogicW()
        {
            ...
            else if (Program.LaneClear && Player.ManaPercent > Config.Item("WMana", true).GetValue<Slider>().Value && Config.Item("farmW", true).GetValue<bool>() && Player.Mana > RMANA + WMANA)
            {
                var minionList = Cache.GetMinions(Player.ServerPosition, W.Range);
                var farmPosition = W.GetLineFarmLocation(minionList, W.Width);

                if (farmPosition.MinionsHit >= Config.Item("WLCminions", true).GetValue<Slider>().Value)
                ...
                if (Q.Instance.Name == "VelkozQ" && Utils.TickCount - Q.LastCastAttemptT > 150)
                {
                    if (Program.LagFree(1) || Program.LagFree(2))
                    {
                        ...
                        if (!Program.None && Player.Mana > RMANA + QMANA)
                        {
                            ...
                            foreach (var enemy in HeroManager.Enemies.Where(enemy => enemy.IsValidTarget(QDummy.Range) && !Common.CanMove(enemy)))
                                CastQ(t);
                        }
                    }
                }
