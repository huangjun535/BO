...
/*
 ...
 Champion.cs is part of YuLeViktor.
...
using YuLeViktor.Enumerations;
using YuLeViktor.Interfaces;
using YuLeViktor.Library.Logger;
using YuLeViktor.Managers;
using YuLeViktor.Menus;
using MinionManager = YuLeViktor.Library.MinionManager;
using MinionOrderTypes = YuLeViktor.Library.MinionOrderTypes;
using MinionTeam = YuLeViktor.Library.MinionTeam;
using MinionTypes = YuLeViktor.Library.MinionTypes;
using Orbwalking = YuLeViktor.STargetSelector.Orbwalking;
using Spell = YuLeViktor.Wrappers.Spell;
using TargetSelector = YuLeViktor.STargetSelector.TargetSelector;
...
namespace YuLeViktor.Abstracts
{
    internal abstract class Champion : IChampion
    {
        ...
        public Menu SMenu { get; private set; }
        ...
        private void SetupMenu()
        {
            try
            {
                SMenu = new Menu("QQ群：438230879", "sfx", true);

                Menu = new Menu("QQ群：438230879", SMenu.Name + "." + Player.ChampionName, true);

                TargetSelector.AddToMenu(SMenu.AddSubMenu(new Menu("Target Selector", SMenu.Name + ".ts")));

                Orbwalker = new Orbwalking.Orbwalker(SMenu.AddSubMenu(new Menu("Orbwalker", SMenu.Name + ".orb")));

                KillstealManager.AddToMenu(SMenu.AddSubMenu(new Menu("Killsteal", SMenu.Name + ".killsteal")));

                var itemMenu = SMenu.AddSubMenu(new Menu("Items", SMenu.Name + ".items"));
                TearStackManager.AddToMenu(itemMenu.AddSubMenu(new Menu("Tear Stacking", SMenu.Name + ".tear-stack." + Player.ChampionName)), Spells);
                ItemManager.AddToMenu(itemMenu, ItemFlags);
                SummonerManager.AddToMenu(SMenu.AddSubMenu(new Menu("Summoners", SMenu.Name + ".summoners")));
                SkinManager.AddToMenu(SMenu.AddSubMenu(new Menu("换肤", SMenu.Name + ".Skins")));
                WardManager.AddToMenu(SMenu.AddSubMenu(new Menu("眼位", SMenu.Name + ".Wardss")));
                Menu.AddToMainMenu();
                SMenu.AddToMainMenu();
