...
namespace Pantheon
{
    internal class Program
    {
        ...
        private static void GameOnOnGameLoad()
        {
            ...
            Config = new Menu("QQ群438230879", "Pantheon", true);
            ...
            Config.AddSubMenu(new Menu("骚扰模式", "Harass"));
            {
            ...
            var menuLane = new Menu("清线模式", "Lane Mode");
            {
                menuLane.AddItem(
                    new MenuItem("Lane.UseQ", "Use Q").SetValue(
                        new StringList(new[] {"关闭", "开启", "只用于普攻范围外"}, 1)));
                menuLane.AddItem(new MenuItem("Lane.UseE", "Use E").SetValue(false));

                menuLane.AddItem(
                    new MenuItem("Lane.Mana.Option", "法力值选项").SetValue(
                        new StringList(new[] {"不检测", "Min. Mana Percent", "保护我的法力"}))
                ...
                menuLane.AddItem(new MenuItem("Lane.Mana.KeepQ", Tab + "Q 技能").SetValue(true));
                menuLane.AddItem(new MenuItem("Lane.Mana.KeepW", Tab + "W 技能").SetValue(true));
                menuLane.AddItem(new MenuItem("Lane.Mana.KeepE", Tab + "E 技能").SetValue(true));
                menuLane.AddItem(new MenuItem("Lane.Mana.KeepR", Tab + "R 技能").SetValue(true));


                menuLane.AddItem(
                    new MenuItem("Lane.Exec", "清线按键").SetValue(new KeyBind("V".ToCharArray()[0],
            ...
            Config.AddSubMenu(menuJungle);
            {
                menuJungle.AddItem(
                    new MenuItem("Jungle.UseQ", "Use Q").SetValue(
                        new StringList(new[] {"Off", "On", "只对大型野怪"}, 1)));
                menuJungle.AddItem(
                    new MenuItem("Jungle.UseW", "Use W").SetValue(
                        new StringList(new[] {"Off", "On", "只对大型野怪"}, 1)));
                menuJungle.AddItem(
                    new MenuItem("Jungle.UseE", "Use E").SetValue(
                        new StringList(new[] {"Off", "当野怪数量 > =1", "当野怪数量 > =2", "当野怪数量 > =3"}, 2)));

                menuJungle.AddItem(
                    new MenuItem("Jungle.Mana.Option", "法力值选项").SetValue(
                        new StringList(new[] {"不检查", "Min. Mana Percent", "为了连招保护我的法力"}))
                        .SetFontStyle(FontStyle.Regular, SharpDX.Color.Aqua)).ValueChanged +=
                    (sender, eventArgs) =>
                    {
                ...
                menuJungle.AddItem(new MenuItem("Jungle.Mana.KeepQ", Tab + "Q 技能").SetValue(true));
                menuJungle.AddItem(new MenuItem("Jungle.Mana.KeepW", Tab + "W 技能").SetValue(true));
                menuJungle.AddItem(new MenuItem("Jungle.Mana.KeepE", Tab + "E 技能").SetValue(true));
                menuJungle.AddItem(new MenuItem("Jungle.Mana.KeepR", Tab + "R 技能").SetValue(true));

                menuJungle.AddItem(
                    new MenuItem("Jungle.Mana.Dont.Title", "如果能杀死以下怪物不考虑魔法损耗").SetFontStyle(
                        FontStyle.Regular, SharpDX.Color.IndianRed));
                menuJungle.AddItem(new MenuItem("Jungle.Mana.Dont.Blue", Tab + "蓝BUFF").SetValue(true));
                menuJungle.AddItem(new MenuItem("Jungle.Mana.Dont.Red", Tab + "红BUFF").SetValue(false));
                menuJungle.AddItem(new MenuItem("Jungle.Mana.Dont.Dragon", Tab + "小龙").SetValue(true));
                menuJungle.AddItem(new MenuItem("Jungle.Mana.Dont.Baron", Tab + "大龙").SetValue(true));


                menuJungle.AddItem(
                    new MenuItem("JungleFarmActive", "打野模式").SetValue(new KeyBind("V".ToCharArray()[0],
                        KeyBindType.Press))).SetFontStyle(FontStyle.Regular, SharpDX.Color.GreenYellow);
            }

            menuMisc = new Menu("杂项模式", "Misc");
            {
        ...
        private static void DoCast(Obj_AI_Base sender, GameObjectProcessSpellCastEventArgs args)
        {
            ...
            if (args.Target is Obj_AI_Hero && Orbwalker.ActiveMode == Orbwalking.OrbwalkingMode.Combo)
            {
                ...
                if (E.IsReady() && !Player.HasBuff("sound") && !Q.IsReady() && !W.IsReady()) E.Cast(target.Position);
            }
        }
        ...
        public static void Obj_AI_Base_OnProcessSpellCast(Obj_AI_Base sender, GameObjectProcessSpellCastEventArgs arg)
        {
            ...
            else if (arg.SData.Name.ToLower().Contains("pantheone") || Player.HasBuff("sound"))
            {
                usedSpell = true;
            }
        ...
        private static void Harass()
        {
            ...
            if (useE && !Player.HasBuff("sound") && !Q.IsReady() && !W.IsReady())
            {
