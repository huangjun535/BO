...
namespace YuLeTeemo
{
    internal class StateManager
    {
        ...
        {
            var enemies = HeroManager.Enemies.FirstOrDefault(t => t.IsValidTarget() && Essentials.Orbwalker.InAutoAttackRange(t));
            ...
            var useWMp = ObjectManager.Player.ManaPercent >= Essentials.Config.SubMenu("Combo").Item("wcombomp").GetValue<Slider>().Value;

            if (Essentials.W.IsReady() && useW && useWMp && !wCombat)
            {
                Essentials.W.Cast();
            }
        ...
        public static void JungleClear()
        {
            ...
            var qManaManager = Essentials.Config.SubMenu("JungleClear").Item("qManaManager").GetValue<Slider>().Value;

            if (Essentials.Orbwalker.ActiveMode == Orbwalking.OrbwalkingMode.LaneClear)
            {
                var jungleMobQ = MinionManager.GetMinions(ObjectManager.Player.Position, Essentials.Q.Range, MinionTypes.All, MinionTeam.Neutral, MinionOrderTypes.MaxHealth);

                if (useQ && jungleMobQ.Where(x => !x.CharData.BaseSkinName.Contains("mini")).FirstOrDefault() != null)
                {
                    if (Essentials.Q.IsReady() && qManaManager <= (int) Essentials.Player.ManaPercent)
                    {
                        Essentials.Q.CastOnUnit(jungleMobQ.Where(x => !x.CharData.BaseSkinName.Contains("mini")).FirstOrDefault());
                    }
                }

                if (useR && jungleMobQ.FirstOrDefault() != null)
                {
                    if (Essentials.R.IsReady() && ammoR >= 1)
                    {
                        Essentials.R.Cast(jungleMobQ.FirstOrDefault().Position);
                    }
                }
            }
        }
