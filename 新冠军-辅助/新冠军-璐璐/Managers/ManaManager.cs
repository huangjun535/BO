...
namespace TreeLib.Managers
{
    public static class ManaManager
    {
        ...
        {
            _menu = menu.AddMenu("ManaManager", "蓝量管理");
            _menu.AddBool("Enabled", "启动!", false);
        }
        ...
        public static void SetManaCondition(this Spell spell, ManaMode mode, int value)
        {
            ...
            {
                _menu.SubMenu(m).AddBool(m + "Enabled", "启动!");
                _menu.SubMenu(m).AddInfo(m + "Info", "-- 技能设置 ", Color.AliceBlue);
            }

            var item = _menu.SubMenu(m)
                .AddSlider(ObjectManager.Player.ChampionName + spell.Slot + "Mana", spell.Slot + " 最低蓝量比", value);
            item.ValueChanged +=
